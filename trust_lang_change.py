...
class Trust_script:
	...
	#topics = "cat_fb22_all_500t_cp_w"
	#msg_tables = ["msg_1001_nrt_core_1", "msg_1001_nrt", "msg_1001_nrt_core_2"] 
	#feat_tables = [ "feat$"+topics+"$"+ msg_table +"$user_id$16to16" for msg_table in  msg_tables ]
	#num_of_topics = 500
	...
	word_to_int_dict = {}
        ...
        def main(self):
                trust_df = self.retrieve_trust()
                print 'size ::::::: ' , sys.getsizeof(trust_df)
                #print trust_df
		...
		self.all_users_indices = { two_hops_users_list[i] : i for i in xrange(len(two_hops_users_list)) }
		...
		network = self.retrieve_network(two_hops_users_list, three_hops_users_list)
		...
		trust_df , random_friends = self.fill_trust(trust_df, network, three_hops_users_list)
		...
		#### retrieve topics ( here words ) by filling self.time_user_topic. As well as getting a list of words that should be later dropped from calculation.	
                delete_list = self.retrieve_topics(two_hops_users_list, three_hops_users_list)
	...
	def retrieve_trust(self):
                print 'db:retrieve_trust'
                data = []
                columns = []
                try:
	...
	def write_to_file(self, data, file_name, thresh = 0):
        ...
        def cosine_sim(self, delta_tt, delta_tf, delta_tr, delete_list):
                ...
                cos_sim_r = []
		order = []
                for i in xrange(0,delta_tt.shape[1]):
                        if i in delete_list:
                                continue
        ...
        def fill_trust(self, trust_df, network, three_hops_users_list):
                print 'fill_trust'
                ...
                for user_id in users_id:
                        ...
                        random_indices = random.sample(range(num_3hops_users), k = len(friends)) #k=int(df.num_of_friends.loc[user_id]) )
                ...
                return trust_df, random_friends
        ...
        def retrieve_network(self, two_hops_users_list, three_hops_users_list):
			...
			for i in xrange(len(two_hops_users_list)):
        ...
        def retrieve_topics(self, two_hops_users_list, three_hops_users_list):
			...
			#### for loop over all three feature tables ( 1st part, 2nd part, total )
                        for i in xrange(len(self.feat_tables)):
			...
			print 'counter: ' , counter
                        ...
                        for i in xrange(len(self.feat_tables)):
                                print 'i: ' , i
                                sql = "select * from {0}".format(self.feat_tables[i])
                                self.cursor.execute(sql)
                                result = self.cursor.fetchall()
                                counter = 0
                                ll = len(result)
			...
			#### so we drop those words having 0 variance, by adding them to the delete_list
                        ...
                        for k in xrange(self.time_user_topic.shape[2]):
                                if k in delete_list:
                                        continue
                                for i in xrange(self.time_user_topic.shape[0]):
                                        if variance[i,k] == 0:
